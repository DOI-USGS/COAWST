function create_inwave_bry(LP, MP, Nangle_bnd, Dir_bnd, obc, ...
...
etime_dimID = netcdf.defDim(nc_bndry,'ocean_time',length(time));
...
etID = netcdf.defVar(nc_bndry,'ocean_time','double',etime_dimID);
netcdf.putAtt(nc_bndry,etID,'long_name','time for energy envelope');
netcdf.putAtt(nc_bndry,etID,'units','seconds');
netcdf.putAtt(nc_bndry,etID,'field','ocean_time, scalar, series');
...
%   Southern boundary
%
  ACID = netcdf.defVar(nc_bndry,'AC_south','double',[xrhodimID eangle_dimID etime_dimID]);
...
%   Eastern boundary
%
  ACID = netcdf.defVar(nc_bndry,'AC_east','double',[erhodimID eangle_dimID etime_dimID]);
...
%   Northern boundary
%
  ACID = netcdf.defVar(nc_bndry,'AC_north','double',[xrhodimID eangle_dimID etime_dimID]);
...
%   Western boundary
%
  ACID = netcdf.defVar(nc_bndry,'AC_west','double',[erhodimID eangle_dimID etime_dimID]);
...
ncwrite(bndfile,'ocean_time',time(1,1:NT));
