...
\chap{About this manual} \label{ch:intro}
...
\chap{General description and instructions for use} \label{ch:defin}
...
\noindent
With this optional command the user indicates that the wind is constant or wants to specify the wind drag coefficient.
\begin{tabbing}
xxxxxxxxxxxx\= \kill
...
{\tt DRAG}    \> indicates the wind drag formulation.\\
{\tt WU}      \> drag coefficient is based on Wu (1982).\\
{\tt FIT}     \> drag coefficient is based on 2nd order polynomial fit.\\
\end{tabbing}
Both quantities {\tt [vel]} and {\tt [dir]} are required if this command is used except when
the command {\tt READINP WIND} is specified.
SWAN converts $U _{10}$ to $U _{*}$ either using the well-known Wu formulae (1982) or
the second order polynomial fit (see Zijlema et al., 2012).
...
{\tt [ygc]}       \> if SWAN is used with \underline{Cartesian coordinates}:\+\\
                     ...
                     Default: the location of the first spectrum encountered in the nest file.\-\\
{\tt [lwdate]}    \> length of character string for date-time as used in the WAM files. Possible values\+\\
                     are: 10 (i.e. {\tt YYMMDDHHMM}), 12 (i.e. {\tt YYMMDDHHMMSS}) or 14 (i.e. {\tt YYYYMMDDHHMMSS}).\\
                     Default: {\tt [lwdate]} = 12\-\\
\end{tabbing}
...
\noindent
With this command the user can influence whitecapping which is usually included in the computations. Can be de-activated with command {\tt OFF WCAP}.
\begin{tabbing}
xxxxxxxxxxxx\= \kill
{\tt KOMEN}   \> whitecapping according to Komen et al. (1984) is applied.\\
{\tt [cds2]}  \> coefficient for determining the rate of whitecapping dissipation (=$C_{\rm ds}$).\+\\
                 Default: {\tt [cds2]} = 2.36e-5.\-\\
{\tt [stpm]}  \> value of the wave steepness for a Pierson-Moskowitz spectrum (=$\tilde{s}^2_{\rm PM}$).\+\\
                 Default: {\tt [stpm]} = 3.02e-3.\-\\
{\tt [powst]} \> power of steepness normalized with the wave steepness of a Pierson-Moskowitz\+\\
                 spectrum.\\
                 Default: {\tt [powst]} = 2.\-\\
{\tt [delta]} \> coefficient which determines the dependency of the whitecapping on wave number.\+\\
                 Default: {\tt [delta]} = 1.\\
                 Note that this default has been changed since version 40.91A. The setting\\
                 {\tt [delta]} = 1 will improve the prediction of the wave energy at low frequencies,\\
                 and hence the mean wave period. The original default was {\tt [delta]} = 0, which\\
                 corresponds to WAM Cycle 3. See the Scientific/Technical documentation for\\
                 further details.\-\\
{\tt [powk]}  \> power of wave number normalized with the mean wave number.\+\\
                 Default: {\tt [powk]} = 1.\-\\
\end{tabbing}
...
%{\tt NKD}      \> indicates that the breaker parameter scales with both the\+\\
%                  bottom slope (=$n$) and the dimensionless depth (=$kd$)\-\\
%{\tt [alpha]}  \> proportionality coefficient of the rate of dissipation.\+\\
%                  Default: {\tt [alpha]} = 1.0.\-\\
%{\tt [gamma0]} \> the reference $\gamma$ for horizontal slopes.\+\\
%                  Default: {\tt [gamma0]} = 0.49.\-\\
%{\tt [alpha0]} \> the upper limit for $\gamma$ for large values of $kd$.\+\\
%                  Default: {\tt [alpha0]} = 1.0.\-\\
%{\tt [beta1]}  \> controls the rate of change of $\gamma$ for the transition\+\\
%                  from low to high values of $kd$.\\
%                  Default: {\tt [beta1]} = 10.0.\-\\
%{\tt [beta2]}  \> controls the value of $kd$ where there is a transition\+\\
%                  from low to high values of $\gamma$ based on $kd$.\\
%                  Default: {\tt [beta2]} = 7.98.\-\\
%{\tt [delta1]} \> controls the upper value of $\gamma$ due to steep slopes at low $kd$.\+\\
%                  Default: {\tt [delta1]} = 0.26.\-\\
%{\tt [delta2]} \> controls the variation of $\gamma$ due to slope $n$;\+\\
%                  particularly seen at low values of $kd$.\\
%                  Default: {\tt [delta2]} = 750.\-\\
%{\tt [n\_low]} \> the lower slope limit (i.e. steep slope limit).\+\\
%                  Default: {\tt [n\_low]} = 2.\-\\
%{\tt [n\_high]}\> the upper slope limit (i.e. gentle slope limit).\+\\
%                  Default: {\tt [n\_low]} = 20.\-\\
%{\tt [R\_low]} \> the relaxation factor for depth for {\tt [n\_low]}.\+\\
%                  Default: {\tt [R\_low]} = 0.000001.\-\\
%{\tt [R\_high]}\> the relaxation factor for depth for {\tt [n\_high]}.\+\\
%                  Default: {\tt [R\_low]} = 50.\-\\
%{\tt [a]}      \> the enhancement of $\gamma$ to account for directional spreading.\+\\
%                  The dimension is rad$^{-1}$.\\
%                  Default: {\tt [a]} = 0.86.\-\\
\end{tabbing}
...
{\tt [xp],[yp]} \> \underline{problem coordinates} of one output location\+\\
                   if Cartesian coordinates are used in m\\
                   if spherical coordinates are used in degrees (see command {\tt COORD})\-\\
{\tt {'fname'}} \> name of the file containing the output locations.\\
\end{tabbing}
...
$ Data produced by SWAN version 40.91A
...
\hl{http://www.swan.tudelft.nl} (Version 40.91A, January 2013).
