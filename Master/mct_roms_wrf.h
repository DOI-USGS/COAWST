...
!
!     Ocean surface current (geographic E/N components)
!
      to_add=':UOCN'
      cad=LEN_TRIM(to_add)
      write(oastring(cid:cid+cad-1),'(a)') to_add(1:cad)
...
# endif
      ...
      END IF
!
!  VOCE Surface current (northward).
!
      DO j=Jstr,Jend
        DO i=IstrR,IendR
# ifdef SOLVE3D
...
# endif
      ...
      END IF
!
!  Now rotate u and v and load into Attr Vect.
!
      ij=0
      DO j=JstrR,JendR
        DO i=IstrR,IendR
...
! u_east  = u_xi * cos(angle) - v_eta * sin(angle)
!
...
# endif
        ...
        END DO
      END DO
      CALL AttrVect_importRAttr (AttrVect_G(ng)%ocn2atm_AV, "UOCN",      &
     &                           A, Asize)
!
      ij=0
      DO j=JstrR,JendR
...
! v_north = u_xi * sin(angle) + v_eta * cos(angle)
!
...
# endif
         ...
         END DO
       END DO
       CALL AttrVect_importRAttr (AttrVect_G(ng)%ocn2atm_AV, "VOCN",     &
				   &                           A, Asize)
#endif
!
