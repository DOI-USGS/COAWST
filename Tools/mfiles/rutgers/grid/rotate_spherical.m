function R = rotate_spherical(D,nx,ny,DX,DY,Rlon,Rlat,Alpha,Clat,plt)

%
% ROTATE_SPHERICAL: Computes a rotated spherical from specified parameters.
%
% R = spherical_grid (D, nx, ny, DX, DY, Rlon, Rlat, Alpha, Clat, plt)
%
% Given a larger donor grid (D), this function extracts a rotated spherical
% grid centered at (Rlon, Rlat) and rotated by angle "Alpha".
%
% On Input:
%
...
%
%    plt        Switch to plot computed rotated grid (OPTIONAL):
%
%                 plt = 0     do not plot
%                 plt = 1     Mercator projection with m_map
%                 plt = 2     faster (lon,lat) plot so 'ginput' can be used
%
% On Output:
%
...
% It produces a logically rectangular curvilinear orthogonal grid with
% minimal distortion. The computation involves 4 stages:
%
...
% svn $Id: rotate_spherical.m 1156 2023-02-18 01:44:37Z arango $
%=========================================================================%
...
disp(['   rotate_spherical: Isotropy Error (dx/dy)-1 = ',               ...
      num2str((dx/dy)-1.)])
...
if (Lplot > 0)
  ...
  title (['Clat = ', num2str(Clat), blanks(4),                          ...
          'Rlon = ', num2str(Rlon), blanks(4),                          ...
          'Rlat = ', num2str(Rlat), blanks(4),                          ...
