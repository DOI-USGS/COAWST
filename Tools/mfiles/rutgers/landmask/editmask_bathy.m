function editmask_bathy(grid_file, varargin)
...
persistent changed rmask rlon rlat mask depth dplot hplot Lp Mp mx my
...
% land=colormap(summer);sea=colormap(cool);
...
%--------------------------------------------------------------------------
  ...
  case 'refresh'
    ...
    %cdata=depth.*rmask*2+mod(mod(mx,2)+mod(my,2),2)+1;
    cdata=rmask*2+mod(mod(mx,2)+mod(my,2),2)+1;
    %set(dplot,'ddata',depth');
...
%     meand=mean(depthr);
%     stdevd=std(depthr);
%     depth=normalize((depthr-meand)./stdevd,'range',[0,4]);
    rmask=mask;
...
% kept constant regardless of figure size.
    ...
    %cdata=depth.*rmask+mod(mod(mx,2)+mod(my,2),2)+1;
    cdata=rmask*2+mod(mod(mx,2)+mod(my,2),2)+1;
    ...
    dplot=image(gx,gy,depth','cdatamapping','direct');
...
%    hplot=image(gx,gy,cdata','cdatamapping','direct');
    set(gca,'YDir','normal',                                            ...
            'layer','top',                                              ...
            'tickdir','out');
%     caxis([min(min(depth'+cdata')) max(max(depth'+cdata'))])
