...
%              S.reference_time reference time datenum
%              S.reference_date reference date (ISO 8601 UTC format)
...
% svn $Id: obs_read.m 1156 2023-02-18 01:44:37Z arango $
%=========================================================================%
...
% get its datenum. Check first variable 'obs_time' and then 'survey_time'
% if the reference date in the 'units' attribute is not available.  Also
% set reference date in the 'ISO 8601 date and time UTC' format.

TimeAtt = nc_getatt(ncfile, 'units', 'obs_time');
ind = strfind(TimeAtt,'since');
if (isempty(ind))
  TimeAtt = nc_getatt(ncfile, 'units', 'survey_time');
  ind = strfind(TimeAtt,'since');
end  

if (~isempty(ind))
  epoch = datenum(TimeAtt(ind+6:ind+24));
  S.reference_time = epoch;
  S.reference_date = datestr(epoch, 'yyyy-mm-ddTHH:MM:SSZ');
else
  S.reference_time = [];
  S.reference_date = [];  
end
