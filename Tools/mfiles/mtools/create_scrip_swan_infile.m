function create_scrip_swan_infile(bath_file,grid_file,Numx,Numy,cartesian,mask,out_file);

%jcw 30July2014 from srcip_swan to make it a function.

%load the swan grid and bathy
bath=textread(bath_file);
%mask=ones(size(bath));
%mask(bath==9999)=0;
mask=mask.';

grd=textread(grid_file);
