function editscope(grid_file, varargin)

% EDITSCOPE: interactive adjoint sensitivity scope mask editing for ROMS
%
% editscope(grid_file)
% editscope(grid_file, 'f')                   GSHHS
% editscope(grid_file, 'mycoast.mat')         see "get_coast.m"
% editscope(grid_file, 'ijcoast.mat')         see "ijcoast.m"
% editscope(grid_file, clon, clat)            see "get_coast.m"
%
...
%
% Warning: 'editscope' is called recursive to process various scope
%          mask editing tasks.
%
% On Input:
%
...
%    Calls: WRITE_SCOPE and UVP_MASKS functions.
%           BUTTON, RADIOBOX, TEXTBOX, AXISSCROLL,
%

% svn $Id: editscope.m 1156 2023-02-18 01:44:37Z arango $
%=========================================================================%
...
persistent changed Rscope rlon rlat scope hplot Lp Mp mx my
...
% at the 'grid_file' argument. Notice that EDITSCOPE is called
% recursively and 'grid_file' is the editing task.
%==========================================================================
...
%--------------------------------------------------------------------------
  ...
  case 'exit'
    ...
    if (~changed)
      ...
      res=questdlg('The scope mask was changed, save?',FIGURE_NAME);
      switch res
        case 'Yes'
          editscope save;
          disp('Scope mask was saved');
          delete(gcf);
        case 'No'
          disp('Scope mask was NOT saved');
          delete(gcf);
      end
    end
...
% Process optional arguments.
    ...
    DIR=fileparts(Cfile{1});           % others are shadoweda
...
% Get data from GRID structure.
    ...
    if (any(strcmp({I.Variables.Name}, 'scope_rho')))
      scope=nc_read(grid_file, 'scope_rho');
    else
      scope=G.mask_rho;
    end
    Rscope=scope;
