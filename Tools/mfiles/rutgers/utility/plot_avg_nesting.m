function F=plot_avg_nesting(Gnames, Anames, Vname, Tindices, Level, varargin)

%
% PLOT_AVG_NESTING:  Plots requested ROMS time-averaged variable from
%                    nesting Average NetCDF files
%
% F=plot_nesting(Gnames, Anames, Vname, Tindices, Level, ...
%                iplot, perimeter, Caxis)
%
% This function plots requested ROMS nesting variable from input
% Average NetCDF files. The user can plot weakly, montly, seasonal,
% and other desired mean. The input records used to compute the
% variable mean are specified in Tindices and depend on the average
% fequency of input NetCDF files.  This function is very useful when
% debugging or browsing a ROMS application. The plotting is not
% that fancy but it provides enough information for browsing ROMS
% variables very quickly. The location of the minumum is marked
% with a filled magenta circle whereas the maximum is marked with
% a filled magenta square.
%
...
%
%    Anames        ROMS history NetCDF files/URL name (cell array)
%
%    Vname         ROMS NetCDF variable name to plot (string)
%
%    Tindices      Time record indices used to compute requested mean
%                    (vector)
%
...
% svn $Id: plot_avg_nesting.m 1156 2023-02-18 01:44:37Z arango $
%=========================================================================%
...
if (~isstruct(Gnames)),
  if (length(Gnames) ~= length(Anames)),
    error([' PLOT_NESTING - need to provide the same number of files',  ...
           ' in ''Gnames'' and ''Anames''']);
  end
end

Nfiles = length(Anames); 
...
if (~isstruct(Gnames)),
  G = grids_structure(Gnames, Anames);
else
  G = Gnames;
end
...
I = nc_vinfo(char(Anames(1)), Vname);
...
% Read in requested variable (V) from history NetCDF file.
  ...
  if (~recordless && any(Tindices) > Tsize),
    return
  end 
  ...
  if (~isempty(Tname)),
    ...
    for rec = 1:Nrec,
      AvgRec = Tindices(rec);
      Tvalue(rec) = nc_read(char(Anames(n)),Tname,AvgRec);
      Tvalue(rec) = Tvalue(rec)/86400;                    % seconds to days
    end

    Tunits=nc_getatt(char(Anames(n)),'units',Tname);
    if (strfind(Tunits,'seconds since ')),
      basedate = Tunits(14:end);
      epoch    = datenum(basedate);
      Tstring  = datestr(mean(Tvalue+epoch), 0);
      got.date = true;
    end    
  end
  ...
  Favg = zeros([size(X) Nrec]);
  
  for rec = 1:Nrec,

    AvgRec = Tindices(rec);
 
    field = nc_read(char(Anames(n)),Vname,AvgRec,ReplaceValue,PreserveType);

    if (is3d),
      if (Level > 0),
        Favg(:,:,rec) = squeeze(field(:,:,Level));
      else
        [~,~,Favg(:,:,rec)] = nc_slice(G,char(Aname(n)),Vname,Level,AvgRec);
      end
    else
      Favg(:,:,rec) = field;
    end
...
% Plot requested field.
  ...
  F(n).ncname = char(Anames(n));
  F(n).vname  = Vname;
  F(n).record = Tindices;
  F(n).time   = mean(Tvalue);
  F(n).values = V;
  F(n).min    = Vmin;
  F(n).max    = Vmax;
  F(n).x      = X;
    ...
    if (is3d),
      if (~isempty(Tname)),
        my_title = [Vname, ':', blanks(4),                              ...
                    ...
                    'time = ', num2str(mean(Tvalue))];
      else
        my_title = [Vname, ':', blanks(4),                              ...
                    'Level = ', num2str(Level), ',', blanks(4),         ...
                    'Record = ', num2str(Tindices)];
      end  
    else
      if (~isempty(Tname)),
        my_title = [Vname, ':', blanks(4),                              ... 
                    'Record = ', num2str(Tindices), ',', blanks(4)];
      else
        my_title = [Vname, ':', blanks(4),                              ... 
                    'Record = ', num2str(Tindices)];
      end
