...
\begin{itemize}
    ...
    \item the SWAN executable(s) for Microsoft Windows,
...
\hl{http://www.swan.tudelft.nl} or \hl{http://swanmodel.sf.net}.
\\[2ex]
\noindent
On the SWAN home page,
...
see \hl{http://www.activestate.com/activeperl/downloads}. The Perl version
...
\begin{itemize}
   ...
   \item If the user want to run SWAN on a relative large number of processors, a message passing
         ...
         any memory but are connected via an interconnection network
         (e.g. cluster of Linux PC's connected via fast Ethernet switches). The implementation is based on
         the Message Passing Interface (MPI) standard (e.g., MPICH2 distribution, freely available
         for several platforms, such as Linux and Windows, at
   ...
   \item For a proper installation of MPICH2 on Windows (XP/Vista/7), please consult Section \ref{sec:mpich2}.
   \item Consult Chapter~\ref{ch:punswan} for parallel, unstructured mesh simulations on distributed
         memory machines.
\end{itemize}
...
\begin{itemize}
  \item To Windows users:
        \begin{itemize}
           ...
           \item To build SWAN on Windows platforms by means of a Makefile you need a make program.
                 However, such a program is usually not available in MS Windows environment. There are two
                 alternatives to get a make program:
                 \begin{enumerate}
                    \item You can download the Visual Studio 2005/2008/2010 Express for free;
                          \hl{http://www.microsoft.com/express}. This contains tools for building
                          applications. It also contains a make program called {\tt NMAKE.EXE}.
                    \item You can download {\tt Nmake} 1.5 for Win32 that is freely available\\
                          (see \hl{http://support.microsoft.com/default.aspx?scid=kb;en-us;Q132084}).\\ Run the
           ...
           \item This installation currently supports MPICH2 for
                 Windows XP/Vista/7; older versions are probably not supported. See Section \ref{sec:mpich2} for further information.
           \item It is assumed that both the directories {\tt include} and
                 {\tt lib} are resided in\\
                 {\tt C:$\backslash$PROGRAM FILES$\backslash$MPICH2}.
  ...
  \item One of the commands {\tt make ser}, {\tt make omp} and {\tt make mpi}
        must be preceded once by {\tt make config}.
  \item If desirable, you may clean up the generated object files and modules by
        typing {\tt make~clean}. If you want to delete any stray files from a
        previous compilation, just type {\tt make~clobber}.
...
\section{Building with netCDF support} \label{sec:netcdf}

SWAN 40.91A contains extensions that provide netCDF output of spectra and maps of several wave parameters.
Carry out the following steps.
\begin{enumerate}
  \item Make sure that netCDF 4.1.x is compiled and available on your system.
        For details, consult \hl{http://www.unidata.ucar.edu/software/netcdf/docs/netcdf-install}.
        The Fortran interface must have been enabled when netCDF was built.
  \item Define the variable {\tt NETCDFROOT} in file {\tt macros.inc} to point to the netCDF root directory
        (e.g. {\tt NETCDFROOT=/usr/local/netcdf-4.1.3} in case of Linux or \\
        {\tt NETCDFROOT=C:$\backslash$PROGRAM FILES$\backslash$netcdf}
         in case of Windows). This enables to compile netCDF I/O into SWAN.
  \item Build SWAN as usual (e.g. {\tt make mpi}); see Section~\ref{sec:quick}.
\end{enumerate}
%Note to Windows users. Download the file \\
%\hl{ftp://ftp.unidata.ucar.edu/pub/netcdf/contrib/win32/netcdf-4.1.1-win32-bin.zip} which contains 32-bit
%binaries of netCDF 4.1.1 (including Fortran libraries). It is ported to Intel Fortran.

\section{Installation of MPICH2 on Windows XP/Vista/7} \label{sec:mpich2}

SWAN can be built with support for MPI. It is assumed that MPI has been installed already in the Linux environment. However, this is probably not the
case for Windows. This section instructs you how to install MPI in the Windows environment. It is assumed that your OS is either Windows XP, Vista or
7.

\begin{itemize}
   \item Install MPICH2 on your machine using the installer downloaded from the downloads page
         \hl{http://www.mcs.anl.gov/research/projects/mpich2/downloads/index.php?s=downloads}.
   \item During installation select MPICH2 to be installed for {\tt Everyone}.
   \item Also make sure that you keep the default setting for passphrase (usually {\tt behappy}) during the installation.
   \item After installing MPICH2 successfully, you may add rules to your firewall to allow the programs {\tt mpiexec.exe} and
         {\tt smpd.exe} to communicate through firewall. This process depends on your firewall.
   \item You need to add the {\tt bin} folder of MPICH2 (usually {\tt C:$\backslash$PROGRAM FILES$\backslash$MPICH2$\backslash$bin}) to your {\small \tt PATH}.
   \item Next, open an admin command prompt by right-clicking on the command prompt icon and selecting {\tt run as administrator}.
         In the command prompt type the following commands in sequence:\\
         {\tt smpd -install}\\
         {\tt mpiexec -remove}\\
         {\tt mpiexec -register}\\
         {\tt mpiexec -validate} (it should return {\small \tt SUCCESS})\\
         {\tt smpd -status} (it should return 'smpd running on {\tt <hostname>}')\\
         If the last two commands are successfully, then you are ready to run SWAN in parallel. See Chapter \ref{ch:usage}.
\end{itemize}
...
\chap{Compiling and running unstructured mesh SWAN in parallel} \label{ch:punswan}
...
For a proper use, version {\bf 50} of ADCIRC must be employed.
\\[2ex]
For details on the implementation and use of the coupled ADCIRC $+$ SWAN model, go to
\hl{http://www.caseydietrich.com/swanadcirc/}.
\\[2ex]
It is assumed that the unstructured mesh is stored in the file {\tt fort.14} and the associated model domain information
is stored in {\tt fort.15} and that these files are yet available.
...
a multi-core cluster is the need to explicitly decompose the unstructured mesh and input files, e.g. {\tt fort.14} and {\tt fort.15},
into smaller pieces, so that each piece can run on its own core on the cluster.
The program {\tt adcprep} is used to perform the decomposition. The actual mesh partitioning is done by
the well-known METIS package (\hl{http://glaros.dtc.umn.edu/gkhome/views/metis}).
\\[2ex]
\noindent
In order to break up the original input files (which describe the full domain) into smaller pieces (called subdomains), go to the directory
where the input files are located and execute {\tt adcprep}.
You need to specify the number of cores that you plan to use for the simulation. The program will decompose your input files into that number of subdomains
and copy each of them into the corresponding local PE sub-directories. Finally, it will copy the SWAN command file into the local PE sub-directories.
If you decide later that you want to run on more (or fewer) cores, you must perform this step again.
\\[2ex]
\noindent
So, more specifically, run {\tt adcprep} and indicate the number of processors. First, you need to do mesh partition using METIS by selecting
\begin{verbatim}
1. partmesh
\end{verbatim}
Next, rerun {\tt adcprep} and continue with full pre-processing by selecting
\begin{verbatim}
2. prepall
\end{verbatim}
or
...
3. prepspec
\end{verbatim}
and specify the necessary ADCIRC files ({\tt fort.14} and {\tt fort.15}; you may skip the file {\tt fort.19}).
Be sure that the initialisation file {\tt swaninit} has been created before running {\tt adcprep}. You may adapt this initialisation file by renaming
{\tt INPUT} to your own SWAN command file. Otherwise, you may copy your command file to {\tt INPUT}.
\\[2ex]
\noindent
After the mesh and input files have been prepared with {\tt adcprep}, the actual SWAN calculation is performed using {\tt swan.exe}.
