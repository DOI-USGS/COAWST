...
%In order to unify these parameterizations, Van der Westhuysen (2009a,b) proposes an alternative breaker
%index that is based on the local shallow water nonlinearity of the wave field.
%\nocite{Wes09a,Wes09b}
\\[2ex]
\noindent
...
\\[2ex]
\noindent
%\underline{Whitecapping: dissipation on opposing current}\\[2ex]
...
%in the significant wave height. Van der Westhuysen (2009c) proposes a saturation-based whitecapping
...
%of the propagation velocity in spectral space $c_\sigma$:
%\begin{equation}
%  S_{\rm ds,c}(\sigma,\theta) = -C''_{\rm ds} \max{\left[ \frac{c_\sigma(\sigma,\theta)}{\sigma} , 0 \right]}
%  {\left[ \frac{B(k)}{B_{\rm r}} \right]}^{p/2} E(\sigma,\theta)  \ \ .
%  \label{eq:wccur}
%\end{equation}
%The proportionality coefficient is set to $C''_{\rm ds}$ = 1.5. The remaining parameters are given by
%Van der Westhuysen (2007), namely $B_{\rm r} = 1.75 \times 10^{-3}$ and $p$ according to (\ref{eq:expo}).
%\nocite{Ris96H,Wes09c}
%\\[2ex]
%\noindent
...
%\\[2ex]
%\noindent
%Van der Westhuysen (2009b) shows that there appears to be a correlation between the nonlinearity of waves in the
%surf zone, i.e. wave asymmetry, and their breaking probability. A parameter that can represent wave asymmetry in SWAN
%is the biphase $\beta$, the phase angle of the complex bispectrum. This biphase is known to progress from 0 to $-\pi/2$
%(or from small values to high values of Ursell number)
%due to triad nonlinear interaction in a shoaling wave. In SWAN, the biphase is parametrized according to expression
%(\ref{eq3-36}). Van der Westhuysen proposes the following fraction of breaking waves, to be used in (\ref {eq:tg83_2}):
%\begin{equation}
%  Q_b = \left( \frac{\beta}{\beta_{\rm ref}} \right)^n\ \ ,
%\end{equation}
%with $\beta_{\rm ref}$ a tunable reference value. Again, $n$ is a parameter to be determined by calibration. In this way,
%it is assumed that the waves breaking due to their shallow water nonlinearity are distributed proportionally over the
%probability density function of wave heights. Van der Westhuysen (2009a) estimates the reference biphase at
%$\beta_{\rm ref} = -4\pi/9$, and shows that the exponent $n$ varies with the mean local wave steepness $S_{\mathrm{loc}}$
%according to
%\begin{equation}
%  n = 4 - \frac{4}{\pi}\arctan \left[ 500\left( S_{\mathrm{loc}}-\tilde{S}_{\mathrm{loc}} \right) \right]
%  \label{eq:nvar}
%\end{equation}
%in which $S_{\mathrm{loc}}$ is defined as
%\begin{equation}
%  S_{\mathrm{loc}} = \frac{H_{\mathrm{rms}} \tilde{k}}{2 \pi} \ , \ \ \ \ \
%  \tilde{k} = \left[ \frac{\int \!\!\! \int k^{-\frac{1}{2}} E (\sigma,\theta) \,d\sigma\,d\theta}{E_{\mathrm{tot}}} \right]^{-2} ,
%  \label{eq:sloc_vink}
%\end{equation}
%with an average value of $\tilde{S}_{\mathrm{loc}}$ = 0.038. This so-called biphase breaker model of
%Van der Westhuysen is implemented in SWAN using a calibrated proportionality coefficient of $B=0.98$ (Van der Westhuysen, 2009a).
...
\end{equation}
%\\[2ex]
%Similarly, the surf breaking source term for the biphase scaling of Van der Westhuysen (2009) can be implemented using
%expression (\ref{eq:st1}) with
%\begin{equation}
%  {\tilde D} = - \frac{3}{4} \frac{B^3 \tilde{\sigma}}{\sqrt{\pi}\,d} \, \left( \frac{\beta}{\beta_{\rm ref}} \right)^n \, H_{\rm rms}
%             = - \frac{3}{4} \gamma \frac{B^3 \tilde{\sigma}}{\sqrt{\pi}} \, \left( \frac{\beta}{\beta_{\rm ref}} \right)^n \, \sqrt{\cal B}
%\end{equation}
%Note, that $\gamma = H_{\rm max}/d$ is not used in the biphase scaling. (It is just a help variable since we need the parameter $\cal B$.)
%Again, we apply the Newton linearisation (\ref{eq:newlin}):
%\begin{equation}
%   S^{n+1}_{i} \approx \left( \frac{\partial S}{\partial E} \right)_i^n E_i^{n+1} + \left( {\tilde D} - \left( \frac{\partial S}{\partial E} \right)_i^n \right) E_i^n
%\end{equation}
%For the derivative $\partial S/\partial E$, we need the derivative of ${\tilde D}$ to $\cal B$ for the application of (\ref{eq:st4}).
%Since, the biphase $\beta$, as given by
%\begin{equation}
%  \beta = -\frac{\pi}{2} + \frac{\pi}{2}\, \tanh(\frac{0.2}{\rm Ur}) \, , \quad {\rm Ur} \sim H_{\rm rms}
%  \label{eq:biphase}
%\end{equation}
%is a function of ${\cal B}$ as well, we need an expression for the derivative $d \beta/d {\cal B}$. The transcendent formula (\ref{eq:biphase}) may be
%approximated with the following more simpler expression
%\begin{equation}
%  \beta = p + q\,{\cal B}^{-1/4}
%\end{equation}
%with $p$ and $q$ some parameters to be determined.
%As a result, we have
%\begin{equation}
%  \frac{\partial S}{\partial E} |_i = \left [ \frac{3}{2}  - \frac{1}{4} n \left( 1 - \frac{p}{\beta} \right) \right ] {\tilde D} |_i < 0\, , \quad \forall p < 0\, ,
%  \, \, \forall n > 0
%\end{equation}
%The choice of $p$ only influences the numerical stability, not the outcomes! Based on some numerical experiments, we found $p = -5$.
%So,
%\begin{equation}
%   S^{n+1}_{i} = \left [ \frac{3}{2}  - \frac{1}{4} n \left( 1 + \frac{5}{\beta} \right) \right ] {\tilde D} |_i^n E_i^{n+1} -
%   \left [ \frac{1}{2}  - \frac{1}{4} n \left( 1 + \frac{5}{\beta} \right) \right ] {\tilde D} |_i^n E_i^n
%\end{equation}
...
%\bibitem{Wes09a}
%Van der Westhuysen, A.J., 2009: Modelling of depth-induced wave breaking over sloping and horizontal beds,
%{\it Proc. 11th International Workshop on Wave Hindcasting and Forecasting}., JCOMM, {\it in press}.

%\bibitem{Wes09b}
%Van der Westhuysen, A.J., 2009: Modelling of depth-induced wave breaking under finite-depth wave growth
%conditions, {\it J. Geophys. Res}., {\it in press}.

%\bibitem{Wes09c}
%Van der Westhuysen, A.J., 2009: Wave dissipation on opposing current, {\it J. Geophys. Res}., {\it in preparation}.
...
\addcontentsline{toc}{chapter}{Index}
  \item ambient, \hyperpage{2}, \hyperpage{7}, \hyperpage{11, 12},
		\hyperpage{35}, \hyperpage{107}

  \indexspace

  \item bathymetry, \hyperpage{24}, \hyperpage{44}, \hyperpage{87},
		\hyperpage{89}, \hyperpage{109}
  \item bottom, \hyperpage{1--3}, \hyperpage{13--15},
		\hyperpage{22--24}, \hyperpage{36}, \hyperpage{52, 53},
		\hyperpage{89}, \hyperpage{93, 94}, \hyperpage{104},
		\hyperpage{106}, \hyperpage{109--114}
  \item boundary, \hyperpage{3, 4}, \hyperpage{13}, \hyperpage{15},
		\hyperpage{50}, \hyperpage{69, 70}, \hyperpage{72, 73},
		\hyperpage{75--79}, \hyperpage{81}, \hyperpage{88},
		\hyperpage{90}, \hyperpage{94}, \hyperpage{108, 109}
  \item breaking, \hyperpage{1}, \hyperpage{3}, \hyperpage{13--17},
		\hyperpage{20, 21}, \hyperpage{23--25}, \hyperpage{49},
		\hyperpage{66}, \hyperpage{101}, \hyperpage{104, 105},
		\hyperpage{108}, \hyperpage{110}, \hyperpage{114}

  \indexspace

  \item Cartesian, \hyperpage{3}, \hyperpage{10--12}, \hyperpage{74},
		\hyperpage{91}, \hyperpage{93, 94}
  \item co-ordinate, \hyperpage{3}, \hyperpage{7}, \hyperpage{10--12},
		\hyperpage{28, 29}, \hyperpage{58--60}, \hyperpage{76},
		\hyperpage{78}
  \item coastal, \hyperpage{1, 2}, \hyperpage{15}, \hyperpage{46},
		\hyperpage{55}, \hyperpage{83}, \hyperpage{87},
		\hyperpage{102, 103}, \hyperpage{105},
		\hyperpage{107--109}, \hyperpage{112, 113},
		\hyperpage{116}
  \item convergence, \hyperpage{42, 43}, \hyperpage{48}, \hyperpage{50},
		\hyperpage{55--58}, \hyperpage{81, 82},
		\hyperpage{85, 86}, \hyperpage{94}, \hyperpage{112},
		\hyperpage{116}
  \item Courant, \hyperpage{46}
  \item current, \hyperpage{2, 3}, \hyperpage{7}, \hyperpage{10--13},
		\hyperpage{15}, \hyperpage{35}, \hyperpage{38, 39},
		\hyperpage{43, 44}, \hyperpage{47}, \hyperpage{50--53},
		\hyperpage{60}, \hyperpage{65}, \hyperpage{71},
		\hyperpage{84}, \hyperpage{86}, \hyperpage{96},
		\hyperpage{104}, \hyperpage{106, 107},
		\hyperpage{111--113}, \hyperpage{115}
  \item curvi-linear, \hyperpage{2, 3}, \hyperpage{58--60},
		\hyperpage{62}, \hyperpage{72}, \hyperpage{78, 79},
		\hyperpage{87}, \hyperpage{102}, \hyperpage{107}

  \indexspace

  \item dam, \hyperpage{16}, \hyperpage{33}, \hyperpage{35--37},
		\hyperpage{107}, \hyperpage{115}
  \item diffraction, \hyperpage{3}, \hyperpage{10}, \hyperpage{36--39},
		\hyperpage{98}, \hyperpage{102}, \hyperpage{104},
		\hyperpage{108, 109}
  \item diffusion, \hyperpage{44--47}, \hyperpage{98}
  \item dissipation, \hyperpage{1}, \hyperpage{3}, \hyperpage{12},
		\hyperpage{14--16}, \hyperpage{20--24},
		\hyperpage{33, 34}, \hyperpage{66, 67}, \hyperpage{88},
		\hyperpage{101}, \hyperpage{103, 104}, \hyperpage{106},
		\hyperpage{111}, \hyperpage{113, 114}

  \indexspace

  \item filter, \hyperpage{13}, \hyperpage{18}, \hyperpage{38}
  \item flow, \hyperpage{7}, \hyperpage{71}, \hyperpage{103},
		\hyperpage{108}, \hyperpage{110}, \hyperpage{115}
  \item force, \hyperpage{33}, \hyperpage{39}, \hyperpage{60},
		\hyperpage{71--73}, \hyperpage{78, 79}, \hyperpage{89},
		\hyperpage{93}, \hyperpage{96}, \hyperpage{104}
  \item frequency, \hyperpage{7--9}, \hyperpage{11}, \hyperpage{13, 14},
		\hyperpage{16}, \hyperpage{18--23}, \hyperpage{25--27},
		\hyperpage{29}, \hyperpage{31}, \hyperpage{34, 35},
		\hyperpage{41--43}, \hyperpage{47, 48}, \hyperpage{50},
		\hyperpage{52--56}, \hyperpage{63--66},
		\hyperpage{69, 70}, \hyperpage{101}, \hyperpage{109},
		\hyperpage{115}
  \item friction, \hyperpage{1}, \hyperpage{3}, \hyperpage{13--15},
		\hyperpage{17, 18}, \hyperpage{22, 23}, \hyperpage{33},
		\hyperpage{106}, \hyperpage{108, 109}, \hyperpage{111},
		\hyperpage{113, 114}

  \indexspace

  \item garden-sprinkler, \hyperpage{45, 46}, \hyperpage{98}

  \indexspace

  \item harbour, \hyperpage{3}, \hyperpage{39}

  \indexspace

  \item initial, \hyperpage{13}, \hyperpage{15}, \hyperpage{30},
		\hyperpage{51}, \hyperpage{69, 70}, \hyperpage{82}
  \item island, \hyperpage{87, 88}

  \indexspace

  \item Jonswap, \hyperpage{69, 70}

  \indexspace

  \item latitude, \hyperpage{11, 12}
  \item limiter, \hyperpage{42}, \hyperpage{55, 56}, \hyperpage{58}
  \item longitude, \hyperpage{11, 12}

  \indexspace

  \item obstacle, \hyperpage{3}, \hyperpage{35--37}, \hyperpage{39},
		\hyperpage{44, 45}, \hyperpage{61--63}
  \item ocean, \hyperpage{1}, \hyperpage{7, 8}, \hyperpage{11},
		\hyperpage{35}, \hyperpage{39}, \hyperpage{46},
		\hyperpage{102}, \hyperpage{106--112},
		\hyperpage{114, 115}

  \indexspace

  \item plant, \hyperpage{33, 34}
  \item propagation, \hyperpage{1--4}, \hyperpage{10--12},
		\hyperpage{38}, \hyperpage{43}, \hyperpage{45--49},
		\hyperpage{51--53}, \hyperpage{59}, \hyperpage{62},
		\hyperpage{83, 84}, \hyperpage{87}, \hyperpage{89},
		\hyperpage{91--94}, \hyperpage{102},
		\hyperpage{104, 105}, \hyperpage{107},
		\hyperpage{110, 111}, \hyperpage{113}

  \indexspace

  \item quadruplets, \hyperpage{16}, \hyperpage{54}

  \indexspace

  \item recti-linear, \hyperpage{79}, \hyperpage{87}
  \item reflection, \hyperpage{3, 4}, \hyperpage{35}, \hyperpage{37},
		\hyperpage{39}, \hyperpage{41}, \hyperpage{62, 63},
		\hyperpage{110}
  \item refraction, \hyperpage{1}, \hyperpage{3}, \hyperpage{11},
		\hyperpage{37}, \hyperpage{44}, \hyperpage{50},
		\hyperpage{52}, \hyperpage{69}, \hyperpage{93},
		\hyperpage{102, 103}, \hyperpage{108, 109},
		\hyperpage{112}, \hyperpage{115}
  \item regular, \hyperpage{2}, \hyperpage{4}, \hyperpage{7},
		\hyperpage{15}, \hyperpage{33}, \hyperpage{36},
		\hyperpage{93}, \hyperpage{112}, \hyperpage{115}

  \indexspace

  \item set-up, \hyperpage{3, 4}, \hyperpage{24}, \hyperpage{39},
		\hyperpage{71}, \hyperpage{79}, \hyperpage{101}
  \item shoaling, \hyperpage{1}, \hyperpage{3}, \hyperpage{15},
		\hyperpage{104, 105}
  \item SORDUP, \hyperpage{44}, \hyperpage{46}, \hyperpage{83}
  \item spherical, \hyperpage{3}, \hyperpage{11, 12}, \hyperpage{94}
  \item stability, \hyperpage{41, 42}, \hyperpage{46}, \hyperpage{50},
		\hyperpage{56}, \hyperpage{67}, \hyperpage{90},
		\hyperpage{93}
  \item stationary, \hyperpage{2, 3}, \hyperpage{7}, \hyperpage{42--44},
		\hyperpage{46}, \hyperpage{54}, \hyperpage{59},
		\hyperpage{70, 71}, \hyperpage{102}, \hyperpage{104},
		\hyperpage{107}, \hyperpage{116}
  \item steepness, \hyperpage{14}, \hyperpage{19, 20}, \hyperpage{24},
		\hyperpage{35}
  \item swell, \hyperpage{14, 15}, \hyperpage{20}, \hyperpage{22},
		\hyperpage{46, 47}, \hyperpage{103}, \hyperpage{106},
		\hyperpage{114}

  \indexspace

  \item triads, \hyperpage{16}
  \item triangular, \hyperpage{82}, \hyperpage{88}, \hyperpage{90--92}

  \indexspace

  \item unstructured, \hyperpage{4}, \hyperpage{87, 88}, \hyperpage{93},
		\hyperpage{96}, \hyperpage{98}, \hyperpage{102},
		\hyperpage{116}

  \indexspace

  \item vegetation, \hyperpage{33--35}

  \indexspace

  \item WAM, \hyperpage{1}, \hyperpage{13, 14}, \hyperpage{17--20},
		\hyperpage{27}, \hyperpage{41, 42}, \hyperpage{55},
		\hyperpage{65}, \hyperpage{69}, \hyperpage{105--107},
		\hyperpage{113, 114}
  \item WAVEWATCH, \hyperpage{1}, \hyperpage{69}, \hyperpage{113}
  \item whitecapping, \hyperpage{3}, \hyperpage{12}, \hyperpage{14},
		\hyperpage{16}, \hyperpage{19, 20}, \hyperpage{22},
		\hyperpage{35}, \hyperpage{106}, \hyperpage{108},
		\hyperpage{113, 114}
  \item wind, \hyperpage{1--3}, \hyperpage{7}, \hyperpage{12--15},
		\hyperpage{17--22}, \hyperpage{31--33}, \hyperpage{35},
		\hyperpage{41}, \hyperpage{43--47}, \hyperpage{49},
		\hyperpage{51}, \hyperpage{54, 55}, \hyperpage{61},
		\hyperpage{70}, \hyperpage{78}, \hyperpage{87},
		\hyperpage{93}, \hyperpage{101--103},
		\hyperpage{106--116}
