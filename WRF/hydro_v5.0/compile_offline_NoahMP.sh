...
if [[ ! -z $MY_HEADER_DIR ]]; then
    export MY_HEADER_DIR="${MY_HEADER_DIR}"
    export HEADPRE='$(shell echo ${ROMS_APPLICATION} | tr [A-Z] [a-z])'
    export HEADER="${HEADPRE}.h"
    export GLOBAL_DEFS_FILE="${MY_ROMS_SRC}/ROMS/Include/globaldefs.h"
fi
...
#make clean;
rm -f Run/wrf_hydro_NoahMP.exe ; rm -f Run/*TBL ; rm -f Run/*namelist*
