...
MODULE W3ADATMD
  ...
  TYPE WADAT
    ...
    REAL, POINTER         :: STK_STOKES(:,:),                       &
                             STU_STOKES(:,:), STV_STOKES(:,:)
    REAL, POINTER         :: XSTK_STOKES(:,:),                      &
                             XSTU_STOKES(:,:), XSTV_STOKES(:,:)
  ...
  REAL, POINTER           :: SXX(:), SYY(:), SXY(:), TAUOX(:),    &
       ...
       TPMS(:), PHICE(:), TAUICE(:,:)
#ifdef W3_COAWST_MODEL
  REAL, POINTER         :: STK_STOKES(:,:), STU_STOKES(:,:),      &
                           STV_STOKES(:,:)
  REAL, POINTER         :: PHIBRKX(:), PHIBRKY(:)
  REAL, POINTER         :: PHICAPX(:), PHICAPY(:)
  REAL, POINTER         :: WLP(:), QB(:)
...
#ifdef W3_COAWST_MODEL
         WADATS(IMOD)%STK_STOKES (NSEALM,NK) ,                &
         WADATS(IMOD)%STU_STOKES (NSEALM,NK) ,                &
         WADATS(IMOD)%STV_STOKES (NSEALM,NK) ,                &
...
#ifdef W3_COAWST_MODEL
    WADATS(IMOD)%STK_STOKES = UNDEF
    WADATS(IMOD)%STU_STOKES = UNDEF
    WADATS(IMOD)%STV_STOKES = UNDEF
...
9003 FORMAT (' TEST W3DIMA : DIMENSIONS STORED')
  ...
  SUBROUTINE W3XDMA  ( IMOD, NDSE, NDST, OUTFLAGS )
    ...
    ELSE
      ...
      CHECK_ALLOC_STATUS ( ISTAT )
    END IF
    IF ( OUTFLAGS( 6, 17) ) THEN
      ALLOCATE ( WADATS(IMOD)%XSTK_STOKES(NXXX,NK), STAT=ISTAT )
      CHECK_ALLOC_STATUS ( ISTAT )
    ELSE
      ALLOCATE ( WADATS(IMOD)%XSTK_STOKES(1,1), STAT=ISTAT )
      CHECK_ALLOC_STATUS ( ISTAT )
    END IF
    IF ( OUTFLAGS( 6, 18) ) THEN
      ALLOCATE ( WADATS(IMOD)%XSTU_STOKES(NXXX,NK), STAT=ISTAT )
      CHECK_ALLOC_STATUS ( ISTAT )
    ELSE
      ALLOCATE ( WADATS(IMOD)%XSTU_STOKES(1,1), STAT=ISTAT )
      CHECK_ALLOC_STATUS ( ISTAT )
    END IF
    IF ( OUTFLAGS( 6, 19) ) THEN
      ALLOCATE ( WADATS(IMOD)%XSTV_STOKES(NXXX,NK), STAT=ISTAT )
      CHECK_ALLOC_STATUS ( ISTAT )
    ELSE
      ALLOCATE ( WADATS(IMOD)%XSTV_STOKES(1,1), STAT=ISTAT )
      CHECK_ALLOC_STATUS ( ISTAT )
    END IF
#endif
    !
...
#ifdef W3_COAWST_MODEL
    WADATS(IMOD)%XSTK_STOKES = UNDEF
    WADATS(IMOD)%XSTU_STOKES = UNDEF
    WADATS(IMOD)%XSTV_STOKES = UNDEF
...
#ifdef W3_COAWST_MODEL
      STK_STOKES   => WADATS(IMOD)%STK_STOKES
      STU_STOKES   => WADATS(IMOD)%STU_STOKES
      STV_STOKES   => WADATS(IMOD)%STV_STOKES
...
9000 FORMAT (' TEST W3SETA : MODEL ',I4,' SELECTED')
  ...
  SUBROUTINE W3XETA ( IMOD, NDSE, NDST )
    ...
    IF ( AINIT2 ) THEN
      !
#ifdef W3_COAWST_MODEL
      STK_STOKES => WADATS(IMOD)%XSTK_STOKES
      STU_STOKES => WADATS(IMOD)%XSTU_STOKES
      STV_STOKES => WADATS(IMOD)%XSTV_STOKES
