...
# endif
     ...
     &    DOMAIN(ng)%Northern_Edge(tile)) THEN
          DO k=1,N(ng)
            DO i=IstrT,IendT
	      BOUNDARY(ng)%t_north(i,k,itemp)=10.0_r8+12.0_r8*          &
     &              SIN(2.0_r8*pi*time(ng)/(24.0_r8*3600.0_r8*60.0_r8))
              BOUNDARY(ng)%t_north(i,k,isalt)=35.0_r8              
	      DO ised=1,NST 
        	BOUNDARY(ng)%t_north(i,k,idsed(ised))=0.5_r8 
              END DO 
            END DO
          END DO
      END IF
